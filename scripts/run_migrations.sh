...
# CI can connect directly to a PostgreSQL service; local Docker usage can keep
# using docker exec against the project container.
psql_exec() {
    if [ -n "$DB_HOST" ]; then
...
LAST_VERSION=$(psql_exec -t -c "SELECT COALESCE(MAX(version), 0) FROM infra.schema_version;" | tr -d '[:space:]')
...
for file in "$MIGRATIONS_DIR"/V*.sql; do
    ...
    psql_exec -c "INSERT INTO infra.schema_version (version, description) VALUES ($version, '$description');"
